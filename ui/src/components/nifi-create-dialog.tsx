...
// nifi.properties content for ConfigMap (from tmp/nifi.yaml)
const NIFI_PROPERTIES = `    # Licensed to the Apache Software Foundation (ASF) under one or more
    ...
    nifi.flow.configuration.file=/data/nifi/conf/flow.xml.gz

    nifi.flow.configuration.json.file=/data/nifi/conf/flow.json.gz

    nifi.flow.configuration.archive.enabled=true

    nifi.flow.configuration.archive.dir=/data/nifi/conf/archive/
    ...
    nifi.templates.directory=/data/nifi/conf/templates
    ...
    # Repository Encryption properties override individual repository
    ...
    nifi.status.repository.questdb.persist.location=/data/nifi/status_repository
    ...
    # Be aware that changing these properties may affect how your instance can
    ...
    nifi.sensitive.props.key=lLP6p1Bx77P8p18qi7sD88wd
    ...
    larger value will produce more accurate results, while a smaller value may
    be
    ...
    # These properties allow configuring one or more NAR providers. A NAR
    provider retrieves NARs from an external source
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - image: library/zs-nifi:1.20.0
        imagePullPolicy: Always
        name: container-u8z382
        ports:
        - containerPort: 8080
          name: tcp-0
          protocol: TCP
        ...
        - name: volume-s5olqx
          readOnly: true
          mountPath: /opt/nifi-1.20.0/conf/nifi.properties
...
interface NifiCreateDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSuccess?: () => void
}

export function NifiCreateDialog({
  open,
  onOpenChange,
  onSuccess,
}: NifiCreateDialogProps) {
  ...
  const handleCreate = async () => {
    ...
    if (!/^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/.test(instanceName)) {
      toast.error(t('nifi.nameInvalid', 'Name must be a valid Kubernetes resource name'))
      return
    }
    if (!namespace?.trim()) {
      toast.error(t('nifi.namespaceRequired', 'Namespace is required'))
      return
    }
    const portNum = parseInt(nodePort.trim(), 10)
    if (isNaN(portNum) || portNum < NODE_PORT_MIN || portNum > NODE_PORT_MAX) {
      toast.error(t('nifi.portRangeError', 'Port must be between 30000-32767'))
      return
    }

    setIsLoading(true)
    try {
      const yamls = generateNifiYamls(instanceName, namespace.trim(), portNum)
      await applyMultiYaml(yamls)
      toast.success(t('nifi.createSuccess', 'NiFi created successfully'))
      ...
      console.error('Failed to create NiFi', err)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>{t('nifi.createTitle', 'Create NiFi')}</DialogTitle>
          <DialogDescription>
            {t('nifi.createDescription', 'Create a NiFi instance with ConfigMap, Services and StatefulSet')}
          </DialogDescription>
        </DialogHeader>

        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label htmlFor="name">{t('nifi.instanceName', 'Instance Name')}</Label>
            <Input
              id="name"
              value={name}
          ...
          <div className="space-y-2">
            <Label htmlFor="nodePort">{t('nifi.nodePort', '对外服务端口')}</Label>
            <Input
              id="nodePort"
              type="number"
              min={NODE_PORT_MIN}
            ...
            <p className="text-xs text-muted-foreground">
              {t('nifi.nodePortHint', 'NodePort 范围 30000-32767，默认 30885')}
            </p>
          </div>
        </div>
